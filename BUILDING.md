...
To build the provided examples that demonstrate how to use this library, utilize the respective CMake Preset:
...
An executable file is created and can be run using the following command (note that some configuration generators (e.g., Visual Studio) may add a configuration folder (e.g., Debug) to the path):
...
```sh
$ cd <project-directory>
$ cmake -S . -B ./build-debug -DCMAKE_BUILD_TYPE=Debug -DBASICLOG_BUILD_TESTS=ON
$ cmake --build ./build-debug -j 8
